...
// Use (db as any) to bypass stale Prisma generated types.
// The Order model exists in schema.prisma and in the DB at runtime.
// This is safe — TypeScript types lag behind schema migrations on Netlify.
...
export async function POST(req: Request) {
  try {
    ...
    // If logged in, enforce the order belongs to the session user
    ...
    const line_items: Stripe.Checkout.SessionCreateParams.LineItem[] = order.items.map(
      (it: { quantity: number; unitPrice: number; productTitle: string; versionTitle?: string }) => ({
        quantity: it.quantity,
        price_data: {
          currency: (order.currency || "USD").toLowerCase(),
          unit_amount: Math.round(it.unitPrice * 100),
          product_data: {
            name: it.versionTitle
              ? it.productTitle + " (" + it.versionTitle + ")"
              : it.productTitle,
          },
        },
      })
